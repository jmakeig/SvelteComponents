...
/**
 * Denormalizes an `Event`'s `customer`/`workload` reference by resolving it against the
 * respective table and copying over `name`/`label`. A real database would do this join at
 * query time instead of storing it; the mock db denormalizes it onto the row itself.
 */
function resolve_event_refs(event: Event): void {
